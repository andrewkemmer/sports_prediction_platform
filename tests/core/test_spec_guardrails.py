...
def test_registry_size_pinned():
    """B-005: the registry size is pinned at 63 — 15 MLB (14 primaries
    incl. game_level_features + markets meta) and 16 per NNX sport (10
    artifact primaries + markets meta + 3 runner-side OOF/fold stores +
    the r5 §22.1 standalone rolling_brier / features_metadata). Any
    change to the registry is a reviewed registry change, never silent
    growth."""
    import importlib
    mod = importlib.import_module("core.contracts.artifact_schema")
    assert len(mod.REGISTRY) == mod.EXPECTED_REGISTRY_SIZE == 63
