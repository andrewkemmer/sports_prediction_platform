...
def test_model_monitor_features_metadata_is_canonical(tmp_path):
    ...
    fx = json.loads(Path("tests/fixtures/nfl_artifact_schemas.json")
                    .read_text(encoding="utf-8"))
    assert list(on_disk) == list(fx["artifacts"]["model_monitor"]["keys"])
    ...
    for stub in (None, {}, {"generated_for": "", "n_features": 0,
        ...
        with pytest.raises(FeaturesMetadataError):
            write_model_monitor_json(
                tmp_path / "nfl_model_monitor_20260901.json", "20260901",
                [], [], [], [], 0.5, {}, {}, features_metadata=stub)
    assert not list(tmp_path.glob("nfl_model_monitor_*.json"))
