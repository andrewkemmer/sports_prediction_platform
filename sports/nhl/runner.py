...
def run_nhl_production(
    *,
    ...
    try:
        ...
        # --------------------------------------------------------------
        from sports.nhl.features import (
            build_game_features,
...
def _build_oof_market_rows(oof_ml: pd.DataFrame, oof_dist: pd.DataFrame,
    ...
    from sports.nhl.distributions import apply_distribution
...
def _power_rankings_inputs(game_df: pd.DataFrame, study):
    ...
    state."""
    from sports.nhl.features import compute_elo, team_events
...
def _write_slate_shap(out_dir: Path, run_date: str, slate: pd.DataFrame,
    ...
    if slate.empty:
        return []
    from sports.nhl.training import member_matrix
