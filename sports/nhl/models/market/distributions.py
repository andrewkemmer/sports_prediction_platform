...
two-way (OT/SO resolves every game â€” the margin is never 0 in the
...
push mass; half-point lines (the NHL standard Â±1.5, 5.5, 6.5) cannot
...
from sports.nhl.features.build_frame import tree_view
...
def game_distribution(mu_h: float, mu_a: float, sigma_margin: float,
    ...
    # P(regulation away win) â€” explicit p_tie, never fabricated away.
    ...
    # the regulation win shares (a documented, deterministic convention â€”
    ...
    # half-stop lines (the NHL standard Â±1.5, 5.5, 6.5): no push band
