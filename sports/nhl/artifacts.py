"""NHL artifact writers.

Every writer validates its output against the immutable schema fixture
(``tests/fixtures/nhl_artifact_schemas.json``) — filenames, column names
and order, JSON key structure, dtypes, required fields, and null behavior
pinned by the Phase 4 fixture. Writers fail loudly on any mismatch;
nothing is silently altered.

Market meanings stay NHL-specific: the full-game moneyline is two-way
(OT/SO resolves every game); the regulation moneyline is three-way with
an explicit ``p_tie``; integer pucklines/totals carry push mass; the
NHL-standard half lines cannot push.
"""
...
FIXTURE_PATH = Path(__file__).resolve().parents[2] / "tests" / "fixtures" \
    / "nhl_artifact_schemas.json"

_MARKETS_COLS_CACHE: list[str] | None = None


class NHLArtifactError(RuntimeError):
    """Raised when an NHL artifact would violate the schema fixture."""



#: Key sets of the standalone monitor siblings (r5 Â§22.1) â identical to
...
def persist_rolling_brier(payload: dict, target_date_str: str,
    ...
    """Standalone ``nhl_rolling_brier_<date>.json`` (Â§22.1 sibling of the
    ...
    out_path = (Path(out_dir or ".")
                / f"nhl_rolling_brier_{target_date_str}.json")
    validate_record("nhl", "rolling_brier", payload)
    path = Path(out_path)
...
def persist_features_metadata(payload: dict, target_date_str: str,
                              out_dir: Path | str | None = None) -> Path:
    """Standalone ``nhl_features_metadata_<date>.json`` (Â§22.1 sibling;
    ...
    out_path = (Path(out_dir or ".")
                / f"nhl_features_metadata_{target_date_str}.json")
    validate_record("nhl", "features_metadata", payload)
    path = Path(out_path)
    path.write_text(json.dumps(_json_safe(payload), indent=1,
                               allow_nan=False))
    return out_path
...
def _fixture() -> dict:
    if not FIXTURE_PATH.is_file():
        raise NHLArtifactError(f"schema fixture missing: {FIXTURE_PATH}")
...
def markets_columns() -> list[str]:
    """The NHL markets schema from the fixture (base + hockey grids)."""
...
def write_calibration_json(path: Path, moneyline_metrics: dict,
    ...
    """MLB/NFL-shaped calibration artifact (fixture: calibration_json)."""
    ...
    safe = _json_safe(record)
    validate_record("nhl", "calibration", safe)
    path.write_text(json.dumps(safe, indent=1, allow_nan=False))
    return record


def write_goalie_matchup_json(path: Path, goalie_df: pd.DataFrame,
                              slate_df: pd.DataFrame) -> dict:
    """Goalie enrichment contract (fixture: goalie_matchup). Unannounced
    starters are None (the frontend renders TBD) — never fabricated."""
    fx = _fixture()["artifacts"]["goalie_matchup"]
    from sports.nhl.goalie_enrichment import GOALIE_FIELDS
    ...
    for i, row in base.iterrows():
        ...
        for f in GOALIE_FIELDS:
            rec[f] = _json_safe(goalie_df.iloc[i][f]) if f in goalie_df.columns \
                else None
        games.append(rec)
    record = {
        "created_utc": _now_utc(),
    ...
    if list(record.keys()) != list(fx["keys"]):
        raise NHLArtifactError(
            f"goalie_matchup key order mismatch: {list(record.keys())}")
    safe = _json_safe(record)
    validate_record("nhl", "goalie_matchup", safe)
    path.write_text(json.dumps(safe, indent=1, allow_nan=False))
    return record
...
def write_feature_json(path: Path, cov: pd.DataFrame, config_meta: dict,
    ...
    safe = _json_safe(record)
    validate_record("nhl", "feature_v1", safe)
    path.write_text(json.dumps(safe, indent=1, allow_nan=False))
    return record
...
def write_model_monitor_json(path: Path, run_date: str, drift: list[dict],
    ...
    require_features_metadata("nhl", features_metadata)
    ...
    safe = _json_safe(record)
    validate_record("nhl", "model_monitor", safe)
    path.write_text(json.dumps(safe, indent=1, allow_nan=False))
    return record
...
def write_markets_monitor_json(path: Path, run_date: str,
    ...
    """Puck-Line & Totals Monitor artifact (fixture: markets_monitor)."""
    ...
    cards = {
        ...
        "puck_line": _card(oof_market_rows, "p_cover_fair", "y_cover_fair"),
        "derived_ml": _card(oof_market_rows, "p_home_win_derived",
                            "y_home_win"),
    }
    slate_history = []
    for card_key in ("over_under", "puck_line", "derived_ml"):
    ...
    safe = _json_safe(record)
    validate_record("nhl", "run_engine_monitor", safe)
    path.write_text(json.dumps(safe, indent=1, allow_nan=False))
    return record
...
def write_predictions_history_csv(path: Path, oof: pd.DataFrame,
    ...
    validate_record("nhl", "predictions_history", out)
...
def write_power_rankings_csv(path: Path, ratings: dict[str, float],
    ...
    for team, elo in sorted(ratings.items(), key=lambda kv: -kv[1]):
        ...
        rows.append({
            ...
            "run_diff": int(goal_diff.get(team, 0)),
    ...
    validate_record("nhl", "power_rankings", df)
...
def write_markets_csv(path: Path, meta_path: Path,
    ...
    for c in cols:
        if c not in out.columns:
            out[c] = np.nan
    out = out[cols]
    validate_record("nhl", "run_engine_markets", out)
    out.to_csv(path, index=False)
    meta = {
    ...
    validate_record("nhl", "run_engine_markets.meta", meta)
...
def persist_shap_game(frame: pd.DataFrame, run_date: str, game_key: str,
    ...
    if missing:
        raise NHLArtifactError(f"shap_game missing columns: {missing}")
    out = frame[cols]
    out_path = out_dir / f"nhl_shap_game_{run_date}_{game_key}.csv"
    validate_record("nhl", "shap_game", out)
