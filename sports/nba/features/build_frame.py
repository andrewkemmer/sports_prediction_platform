...
NBA-specific derivations (schedule-derivable by design â€” see study.yaml):
...
def team_events(games: pd.DataFrame) -> pd.DataFrame:
    ...
    # Full-game resolution: an NBA game CANNOT tie â€” overtime repeats
...
def team_stats_ladder(events: pd.DataFrame, *, form_window: int,
    ...
    win_pct, rest_days, b2b, ewm_net_pts â€” every trailing value
    ...
    if events["team"].isna().any():
        raise ValueError(
            ...
            f"({int(events['team'].isna().sum())} rows) â€” groupby would "
            "silently drop them")
    ...
    # REAL NBA data â€” same-calendar-day doubleheaders are genuinely
...
def _load_arena_table(path=None) -> pd.DataFrame:
    ...
    if not p.is_file():
        logger.warning("NBA arena table missing at %s â€” travel features "
...
def build_slate_features(schedule: pd.DataFrame) -> pd.DataFrame:
    ...
    # reported â€” never silently dropped and never fabricated.
