...
def persist_rolling_brier(payload: dict, target_date_str: str,
    ...
    """Standalone ``nba_rolling_brier_<date>.json`` (Â§22.1 sibling of the
    ...
    out_path = (Path(out_dir or ".")
                / f"nba_rolling_brier_{target_date_str}.json")
    validate_record("nba", "rolling_brier", payload)
    path = Path(out_path)
...
def persist_features_metadata(payload: dict, target_date_str: str,
                              out_dir: Path | str | None = None) -> Path:
    """Standalone ``nba_features_metadata_<date>.json`` (Â§22.1 sibling;
    ...
    out_path = (Path(out_dir or ".")
                / f"nba_features_metadata_{target_date_str}.json")
    validate_record("nba", "features_metadata", payload)
    path = Path(out_path)
    path.write_text(json.dumps(_json_safe(payload), indent=1,
                               allow_nan=False))
    return out_path
