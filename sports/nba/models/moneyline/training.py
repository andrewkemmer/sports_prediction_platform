"""Production NBA moneyline: five-member ensemble (xgboost / lightgbm /
...
TIE semantics (sport-specific): an NBA game CANNOT end in a tie —
overtime periods repeat until a winner exists. The moneyline is
structurally two-way; there is no tie outcome to model and no tie mass
is ever fabricated.
"""
...
def walk_forward_oof(game_df: pd.DataFrame, study,
    ...
    Returns a dict with: ``oof`` (game_id, game_date, fold_id, per-member
...
def fit_final_models(game_df: pd.DataFrame,
                     study) -> tuple[dict, TrainFoldPreprocessor]:
...
# ---------------------------------------------------------------------------
# Board (slate) moneyline prediction
# ---------------------------------------------------------------------------
