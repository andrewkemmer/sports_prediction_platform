"""NBA Phase 5 production runner.

The ONLY sanctioned end-to-end NBA production path:

1. mandatory run-window validation — ``NBA_START_DATE`` / ``NBA_END_DATE``
   / ``NBA_FULL_REPULL`` must be present and valid (strict YYYY-MM-DD;
   END >= START; FULL_REPULL exactly "0" or "1"; no silent defaults, no
   season-variable aliases — forbidden aliases fail loudly);
2. bounded NBA Stats API ingestion (incremental or full repull per flag);
...
from ``sports/nba/study.yaml`` — NEVER from the run window. The run
...
from sports.nba.features.registry import (
    MARKET_FEATURE_COLS,
    MONEYLINE_FEATURE_COLS,
)

#: §7.1 per-field availability metadata layer (B-001-RESIDUAL, r6): the
#: union of both frozen contracts — every field the slate predictions
#: may consume carries a declared availability class and is gated.
...
ENV_START_DATE = "NBA_START_DATE"
ENV_END_DATE = "NBA_END_DATE"
ENV_FULL_REPULL = "NBA_FULL_REPULL"
FORBIDDEN_ENV_VARS = ("NBA_START_SEASON", "NBA_END_SEASON",
                      "NBA_SLATE_SEASON")

NBA_CACHE = Path("store/nba/raw")


class NBARunnerError(RuntimeError):
    """Raised when the NBA production run cannot proceed safely."""
...
def run_nba_production(
    *,
    ...
    """Execute the bounded NBA production run.
    ...
    if present_forbidden:
        raise NBARunnerError(
            ...
            f"{present_forbidden}. The run window is NBA_START_DATE / "
            f"NBA_END_DATE only — training history comes from study.yaml.")
    ...
    if missing:
        raise NBARunnerError(
            ...
            f"{missing}. NBA_START_DATE, NBA_END_DATE, and "
            f"NBA_FULL_REPULL must be set explicitly — no silent defaults.")
    window = resolve_run_window(start_date=source[ENV_START_DATE],
                                end_date=source[ENV_END_DATE],
                                full_repull=source[ENV_FULL_REPULL])

    cache = Path(cache_dir) if cache_dir else NBA_CACHE
    ...
    sink = Path(data_delivery_dir) if data_delivery_dir \
        else cfg.data_delivery_dir("nba", Path("."))
    try:
        # --------------------------------------------------------------
        ...
        if not dry_run_ingestion:
            pull_schedule(
                ...
                sched_path,
                full_repull=window.full_repull,
                fetch=fetch_schedule or _default_fetch_schedule,
                today=today)
        ...
        from sports.nba.models.moneyline.training import (
            apply_platt,
            fit_final_models,
            fit_platt,
            predict_slate,
        ...
        key_cols = ["game_id", "game_date", "season", "home_score",
                    "away_score", "margin", "total", "venue"]
        if "home_record" in game_df.columns:
            key_cols += ["home_record", "away_record"]
        key = game_df[key_cols].copy()
        ...
        # must carry a rule-consistent win/loss outcome (full-game NBA
        ...
        # evaluation metrics
        from sports.nba.evaluation import (
            binary_metrics,
            calibration_buckets,
        )
        ...
        if not slate.empty:
            ...
            slate["has_offer"] = False
            for c in ("p_cover_offered", "p_push_offered",
                      "p_over_offered", "p_under_offered",
                      "p_push_total_offered"):
                slate[c] = np.nan
        result.n_prediction_games = int(len(slate))

        # Participant enrichment (display only; bounded per-game boxscore
        # pulls)
        player_cache = None
        if not slate.empty and not dry_run_ingestion:
            ...
            gd_map = dict(zip(sched_all["game_id"].astype(str),
                              pd.to_datetime(
                                  sched_all["game_date"]).dt.strftime(
                                  "%Y-%m-%d")))
            ...
            for row in slate.itertuples(index=False):
                ...
                for team in (row.home_team, row.away_team):
                    m = ((sched_all["home_team"] == team)
                         | (sched_all["away_team"] == team))
                    t = sched_all.loc[m & done]
                    ...
                    # slate covers every last-10 window the panel reads;
                    # a live season's mid-slate completions are included)
        ...
        player_df = enrich_slate(slate, player_cache) if not slate.empty \
            else pd.DataFrame()
        ...
        if not slate.empty:
            written.append(write_moneyline_json(
                sink / f"nba_moneyline_v1_{date_c}.json", slate,
                p_home_cal, team_names, config_meta)
                and sink / f"nba_moneyline_v1_{date_c}.json")
            written.append(write_player_matchup_json(
                sink / f"nba_player_matchup_{date_c}.json", player_df,
                slate) and sink / f"nba_player_matchup_{date_c}.json")
            written += _write_slate_shap(sink, date_c, slate, final_models,
                                         weights, study)

        write_calibration_json(
            sink / f"nba_calibration_{date_c}.json", raw_m, cal_m,
        ...
        written.append(sink / f"nba_calibration_{date_c}.json")

        write_predictions_history_csv(
            sink / f"nba_predictions_history_{date_c}.csv", oof_ml,
            oof_ml["p_ensemble_calibrated"].to_numpy())
        written.append(sink / f"nba_predictions_history_{date_c}.csv")

        ratings, records, point_diff = _power_rankings_inputs(game_df, study)
        write_power_rankings_csv(
            sink / f"nba_power_rankings_{date_c}.csv",
            ratings, records, point_diff, team_names)
        written.append(sink / f"nba_power_rankings_{date_c}.csv")

        write_markets_csv(
            sink / f"nba_run_engine_markets_{date_c}.csv",
            sink / f"nba_run_engine_markets_{date_c}.meta.json",
            oof_market_rows, slate, config_meta)
        written.append(sink / f"nba_run_engine_markets_{date_c}.csv")

        # OOF stores (backend evaluation data — retention-exempt dir)
        models_dir = sink / "models"
        models_dir.mkdir(parents=True, exist_ok=True)
        validate_record("nba", "oof_moneyline", oof_ml)
        oof_ml.to_csv(models_dir / f"nba_oof_moneyline_{date_c}.csv",
                      index=False)
        validate_record("nba", "oof_distribution", oof_dist)
        oof_dist.to_csv(models_dir / f"nba_oof_distribution_{date_c}.csv",
                        index=False)
        _fold_tbl = _fold_table(fold_list)
        validate_record("nba", "fold_table", _fold_tbl)
        _fold_tbl.to_csv(
            models_dir / f"nba_fold_table_{date_c}.csv", index=False)
        ...
        write_feature_json(sink / f"nba_feature_v1_{date_c}.json", cov,
                           config_meta, _fold_summary(fold_list), fc_meta)
        written.append(sink / f"nba_feature_v1_{date_c}.json")
        ...
        write_markets_monitor_json(
            sink / f"nba_run_engine_monitor_{date_c}.json", date_c,
            oof_market_rows, config_meta, ml_reference=ml_ref)
        written.append(sink / f"nba_run_engine_monitor_{date_c}.json")

        write_model_monitor_json(
            sink / f"nba_model_monitor_{date_c}.json", date_c, [], [],
        ...
        logger.exception("NBA production run: artifact generation FAILED "
...
def _team_names(schedule: pd.DataFrame) -> dict[str, str]:
    """Pass-through identity map (the NBA API schedule carries tricodes;
    full names come from the shared teams source when a future platform
    table exists — tricodes are never replaced by invented names)."""
...
def _build_oof_market_rows(oof_ml: pd.DataFrame, oof_dist: pd.DataFrame,
    ...
    from sports.nba.models.market.distributions import apply_distribution
...
def _power_rankings_inputs(game_df: pd.DataFrame, study):
    """(ratings dict, records dict, point_diff dict) from the decided Elo
    state."""
    from sports.nba.features.build_frame import compute_elo, team_events
    ...
    rec = ev.groupby("team").agg(
        ...
        point_diff=("net_from_team", "sum"),
    )
    records = {t: (int(rec.loc[t, "wins"]), int(rec.loc[t, "losses"]))
               for t in rec.index}
    point_diff = {t: int(rec.loc[t, "point_diff"]) for t in rec.index}
...
def _daily_rows(oof_ml: pd.DataFrame) -> list[dict]:
    ...
    from sports.nba.evaluation import binary_metrics
    daily = []
    oof = oof_ml.copy()
...
def _write_slate_shap(out_dir: Path, run_date: str, slate: pd.DataFrame,
                      ...
                      study) -> list[Path]:
    """Emit EXACTLY one nba_shap_game_<date>_<game_id>.csv per undecided
    ...
    if slate.empty:
        return []
    from sports.nba.models.moneyline.training import member_matrix
    ...
    if not contribs:
        logger.warning("no tree member produced SHAP contributions — no "
                       "nba_shap_game files written (nothing fabricated)")
        return []
    ...
    if len(written) != len(slate):
        raise NBARunnerError(
            f"nba_shap_game count rule violated: {len(written)} files for "
            f"{len(slate)} slate games")
    keys = [p.name for p in written]
    if len(set(keys)) != len(keys):
        raise NBARunnerError("nba_shap_game keys are not unique per game")
    return written
...
def _run_and_log() -> int:
    ...
    log = logging.getLogger("nba.run_production")
    up = "nba".upper()
    try:
