...
Architecture (ported reference-exact): a JOINT score distribution.
...
TIE/PUSH semantics: the derived moneyline is THREE-way (p_home_win +
p_tie + p_away_win = 1, with p_tie the tied-margin mass); integer spread
lines and integer total lines carry explicit push mass; half-point lines
cannot push (margins/totals are integers).
"""
...
from sports.nfl.features.build_frame import tree_view
...
def margin_cdf_above(pmf: np.ndarray, support: np.ndarray, L: float) -> float:
    ...
    L, and m >= L + 1 for integer L. (NFL margins are integers; the
...
class ScoreRegressor:
    ...
    def _matrix(self, df: pd.DataFrame, study) -> pd.DataFrame:
        ...
        # contract builders. See sports/nfl/features.py::tree_view.
...
def calibrate_sigma(resid_margin: np.ndarray, resid_total: np.ndarray,
    ...
    mc, mk = study.market, study.market
...
def game_distribution(mu_h: float, mu_a: float, sigma_margin: float,
    ...
    # derived moneyline: THREE-way — P(home win), P(tie), P(away win)
    p_home_win = margin_cdf_above(pmf_m, support_m, 0.0)
    p_tie = margin_pmf_at(pmf_m, support_m, 0.0)
