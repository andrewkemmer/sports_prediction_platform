"""NFL artifact writers.

Every writer validates its output against the immutable schema fixture
(``tests/fixtures/nfl_artifact_schemas.json``) — filenames, column names
and order, JSON key structure, dtypes, required fields, and null behavior
pinned to the read-only reference repository's real production artifacts.
Writers fail loudly on any mismatch; nothing is silently altered.
"""
...
FIXTURE_PATH = Path(__file__).resolve().parents[2] / "tests" / "fixtures" \
    / "nfl_artifact_schemas.json"

_MARKETS_COLS_CACHE: list[str] | None = None


class NFLArtifactError(RuntimeError):
    """Raised when an NFL artifact would violate the schema fixture."""


def _fixture() -> dict:
    if not FIXTURE_PATH.is_file():
        raise NFLArtifactError(f"schema fixture missing: {FIXTURE_PATH}")
...
def markets_columns() -> list[str]:
    """The 241-column markets schema from the fixture (base + grids)."""
...
def write_moneyline_json(path: Path, slate_df: pd.DataFrame,
    ...
    for i in range(len(slate)):
        ...
        start = f"{gd}T{gt}:00Z" if (gd and ":" in gt) else \
    ...
    record = {
        ...
        "config": config_meta,
        "slate_date": str(slate["gameday"].min()) if len(slate) else None,
        "n_games": len(games),
        "games": games,
    }
    if list(record.keys()) != list(fx["keys"]):
        raise NFLArtifactError(
            f"moneyline_json key order mismatch: {list(record.keys())}")
    safe = _json_safe(record)
    validate_record("nfl", "moneyline_v1", safe)
...
def write_calibration_json(path: Path, moneyline_metrics: dict,
    ...
    """MLB-shaped calibration artifact (fixture: calibration_json)."""
    ...
    safe = _json_safe(record)
    validate_record("nfl", "calibration", safe)
    path.write_text(json.dumps(safe, indent=1, allow_nan=False))
    return record


def write_qb_matchup_json(path: Path, qb_df: pd.DataFrame,
                          slate_df: pd.DataFrame) -> dict:
    """QB enrichment contract (fixture: qb_matchup). Unannounced starters
    are None (the frontend renders TBD) — never fabricated."""
    fx = _fixture()["artifacts"]["qb_matchup"]
    from sports.nfl.qb_enrichment import QB_FIELDS
    ...
    for i, row in base.iterrows():
        ...
        for f in QB_FIELDS:
            rec[f] = _json_safe(qb_df.iloc[i][f]) if f in qb_df.columns \
                else None
        games.append(rec)
    record = {
        "created_utc": _now_utc(),
    ...
    if list(record.keys()) != list(fx["keys"]):
        raise NFLArtifactError(
            f"qb_matchup key order mismatch: {list(record.keys())}")
    safe = _json_safe(record)
    validate_record("nfl", "qb_matchup", safe)
    path.write_text(json.dumps(safe, indent=1, allow_nan=False))
    return record
...
def write_feature_json(path: Path, cov: pd.DataFrame, config_meta: dict,
    ...
    safe = _json_safe(record)
    validate_record("nfl", "feature_v1", safe)
    path.write_text(json.dumps(safe, indent=1, allow_nan=False))
    return record
...
def write_model_monitor_json(path: Path, run_date: str, drift: list[dict],
    ...
    safe = _json_safe(record)
    validate_record("nfl", "model_monitor", safe)
    path.write_text(json.dumps(safe, indent=1, allow_nan=False))
    return record
...
def write_markets_monitor_json(path: Path, run_date: str,
    ...
    """Run-Line & Totals Monitor artifact (fixture: markets_monitor)."""
    ...
    cards = {
        ...
        "run_line": _card(oof_market_rows, "p_cover_fair", "y_cover_fair"),
    ...
    safe = _json_safe(record)
    validate_record("nfl", "run_engine_monitor", safe)
    path.write_text(json.dumps(safe, indent=1, allow_nan=False))
    return record
...
def write_predictions_history_csv(path: Path, oof: pd.DataFrame,
    ...
    out = pd.DataFrame({
        ...
        "actual_winner": np.select(
            ...
            [df["home_team"], df["away_team"]], default="TIE"),
    })
    ...
    validate_record("nfl", "predictions_history", out)
...
def write_power_rankings_csv(path: Path, ratings: dict[str, float],
    ...
    validate_record("nfl", "power_rankings", df)
...
def write_markets_csv(path: Path, meta_path: Path,
    ...
    for c in cols:
        if c not in out.columns:
            out[c] = np.nan
    out = out[cols]
    validate_record("nfl", "run_engine_markets", out)
    out.to_csv(path, index=False)
    meta = {
    ...
    validate_record("nfl", "run_engine_markets.meta", meta)
...
def persist_shap_game(frame: pd.DataFrame, run_date: str, game_key: str,
    ...
    if missing:
        raise NFLArtifactError(f"shap_game missing columns: {missing}")
    out = frame[cols]
    out_path = out_dir / f"nfl_shap_game_{run_date}_{game_key}.csv"
    validate_record("nfl", "shap_game", out)
